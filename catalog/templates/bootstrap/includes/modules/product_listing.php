<?php
/*
$Id: product_listing.php 2007 2014-02-15 20:22:44Z cottonbarn $
...
<!--[if lte IE 8]>
<style>
img.corner_banner { display:inline-block; margin-left:-<?php echo (($gridlist == 'list') ? '7' : '105'); ?>px; margin-top:-7px; position:absolute; } 
</style>
<![endif]-->
...
if ($gridlist != 'list') {
// WARNING the code assumes there are three products per row.
  $counter = $row;
      ...
      for ($col=0, $n=sizeof($column_list); $col<$n; $col++) {
        ...
        switch ($column_list[$col]) {
          ...
          case 'PRODUCT_LIST_NAME':
            ...
            $lc_align = '';
            if (isset($_GET['manufacturers_id'])) {
              $lc_text = '<a href="' . tep_href_link(FILENAME_PRODUCT_INFO, 'manufacturers_id=' . $_GET['manufacturers_id'] . '&products_id=' . $listing[$x]['products_id']) . '">' . $listing[$x]['products_name'] . (($gridlist == 'list') ? $short : '') /*begin epf*/ . $extra /*end epf*/ . '</a>';
            } else {
...
// EOF QPBPP for SPPC
          ...
          case 'PRODUCT_LIST_IMAGE':
            $lc_align = 'center';
            if (isset($_GET['manufacturers_id'])) {
              $lc_text = '<div class="placeholder"><a href="' . tep_href_link(FILENAME_PRODUCT_INFO, 'manufacturers_id=' . $_GET['manufacturers_id'] . '&products_id=' . $listing[$x]['products_id']) . '">' . tep_image(DIR_WS_IMAGES . DYNAMIC_MOPICS_THUMBS_DIR . $listing[$x]['products_image'], $listing[$x]['products_name'], SMALL_IMAGE_WIDTH, SMALL_IMAGE_HEIGHT) . '</a></div>';
            } else {
              $lc_text = '<div class="placeholder">&nbsp;<a href="' . tep_href_link(FILENAME_PRODUCT_INFO, ($cPath ? 'cPath=' . $cPath . '&' : '') . 'products_id=' . $listing[$x]['products_id']) . '">' . tep_image(DIR_WS_IMAGES . DYNAMIC_MOPICS_THUMBS_DIR . $listing[$x]['products_image'], $listing[$x]['products_name'], SMALL_IMAGE_WIDTH, SMALL_IMAGE_HEIGHT) . '</a>&nbsp;</div>';
            }
            break;
          case 'PRODUCT_LIST_BUY_NOW':
            $lc_align = 'center';
